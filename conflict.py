...
class ConflictRelaxer:
                ...
                for level, relaxing_level in enumerate(sorted(iis_priority_set, key=lambda x: x.value)):
                    ...
                    try:
                        slack_dict = self.relax_iis(iis, relaxer_objective=relaxer_objective,
                                                    lowest_priority=relaxing_level)
    ...
    def relax_iis(
            ...
            lowest_priority: 'mip.constants.ConstraintPriority' = None
    ) -> dict:
        ...
        if relax_iis_model.status == mip.OptimizationStatus.INFEASIBLE:
            raise SubRelaxationInfeasible(
                "sub relaxation model infeasible, this could mean that in the IIS the mandatory constraints are infeasible sometimes")
